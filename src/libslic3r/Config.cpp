...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    bool has_delimiters = true;
    {
        ...
        // Upstream Snapmaker #743 (1ea4b31407): tiene que ir a la par con GCodeProcessor::Producers.
        // El gcode de OrcaSlicer de serie usa nuestro mismo formato de bloque de config, y rechazar
        // su cabecera aqui abortaba la importacion entera aunque alli el productor ya se reconozca.
        ...
        while (std::getline(ifs, line)) {
            ...
            if (line.find("; THUMBNAIL_BLOCK_START") != std::string::npos) {
                ...
                if (line.find("; thumbnail begin") != std::string::npos) {
                    ...
                    // Upstream Snapmaker #743 (1ea4b31407): solo existen thumb0 y thumb1 como opcion.
                    // El gcode de OrcaSlicer puede traer mas bloques de miniatura; saltarse los que no
                    // tienen opcion con un aviso, en vez de dejar que set_deserialize aborte la
                    // importacion entera con un unknown-option.
