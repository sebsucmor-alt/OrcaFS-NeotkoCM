...
enum SurfaceType {
    ...
    stInternalSolid,
    // NEOTKO_MULTIPASS_TAG_START: Penultimate solid infill — layers immediately below stTop.
    // Independent density/speed/role (erPenultimateInfill). Preserved through entire pipeline.
    // is_solid_infill() matches this type: always check stPenultimateInternalSolid BEFORE is_solid_infill().
...
public:
	...
	bool   is_solid()    const { return this->is_external() || this->surface_type == stInternalSolid || this->surface_type == stPenultimateInternalSolid || this->surface_type == stInternalBridge; }
	bool   is_solid_infill() const { return this->surface_type == stInternalSolid || this->surface_type == stPenultimateInternalSolid; }
};
