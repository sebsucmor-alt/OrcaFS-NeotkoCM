...
std::pair<float, Point> Fill::_infill_direction(const Surface *surface) const
{
    ...
    if (surface->bridge_angle >= 0) {
        ...
        // When Neoweaving Linear mode is active, suppress per-layer angle rotation for solid infill
        // surfaces so adjacent layers share the same fill angle → Z zigzag interdigitates correctly.
        // stPenultimateInternalSolid: always lock (top-derived surface, part of top surface stack).
        // stInternalSolid with neoweave_filter=All: lock so bottom infill aligns with top surface.
        bool neoweave_lock_angle = false;
        if (this->print_region_config != nullptr
            && this->print_region_config->interlayer_neoweave_enabled.value
            && this->print_region_config->interlayer_neoweave_mode.value == NeoweaveMode::Linear) {
            const NeoweaveFilter flt = this->print_region_config->neoweave_filter.value;
            if (surface->surface_type == stPenultimateInternalSolid)
                neoweave_lock_angle = true;
            else if (surface->surface_type == stInternalSolid && flt == NeoweaveFilter::All)
                neoweave_lock_angle = true;
        }
