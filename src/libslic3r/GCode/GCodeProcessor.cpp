...
#include "../NeoDebug.hpp"  // NEOTKO_TOOLSLEEP_TAG s294
#include <cctype>
...
void GCodeProcessorResult::reset() {
    ...
    custom_gcode_per_print_z = std::vector<CustomGCode::Item>();
    spiral_vase_layers = std::vector<std::pair<float, std::pair<size_t, size_t>>>();
    // NEOTKO_SPIRALGUARD_TAG
    spiral_lift_total = 0;
    spiral_lift_degraded = 0;
    spiral_lift_first_degraded_z = 0.;
...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    ...
    // Upstream Snapmaker #743 (1ea4b31407): OrcaSlicer de serie comparte nuestro formato de bloque
    // de config. Sin esta entrada su cadena de productor no casaba con ninguna de arriba,
    // apply_config se saltaba entero y extruders_count se quedaba en 0 (todas las T rechazadas).
...
void GCodeProcessor::apply_config(const PrintConfig& config)
{
    ...
    // NEOTKO_TOOLSLEEP_TAG s294 — mirrored from app_config through PrintConfig, never from a preset.
    m_neotko_toolsleep_enabled = config.neotko_idle_tool_power_down.value;
    m_neotko_toolsleep_deep    = m_neotko_toolsleep_enabled && config.neotko_idle_tool_deep_sleep.value;
    // sanity check
...
void GCodeProcessor::reset()
{
    ...
    m_preheat_steps = 1;
    // NEOTKO_TOOLSLEEP_TAG s294
    m_neotko_toolsleep_enabled = false;
    m_neotko_toolsleep_deep    = false;
    m_neotko_toolsleep_last_use.clear();
    m_neotko_toolsleep_done.clear();
    m_neotko_toolsleep_rewrites = 0;
...
//
// Why here and not in GCode.cpp: the decision needs the FINAL gcode. OozePrevention::pre_toolchange
// emits the standby M104 without knowing whether the tool ever comes back, and NeoTower can add a
// visit after the fact, so any "last use" computed at plan time can be wrong. run_post_process runs
// after everything is emitted and after m_result.moves is fully parsed, so "does this tool come
// back?" is simply a lookup. Same reasoning as the plan's §5.1.
//
// Fills m_neotko_toolsleep_last_use[tool] with the gcode line id of that tool's last EXTRUDING
// move. 0 means the tool never extrudes in this file.
void GCodeProcessor::neotko_toolsleep_prepare()
{
    m_neotko_toolsleep_last_use.clear();
    m_neotko_toolsleep_done.clear();
    m_neotko_toolsleep_rewrites = 0;
    m_neotko_toolsleep_tools_used = 0;
    if (!m_neotko_toolsleep_enabled)
        return;

    const size_t tools = std::max<size_t>(m_result.extruders_count, 1);
    m_neotko_toolsleep_last_use.assign(tools, 0);
    m_neotko_toolsleep_done.assign(tools, false);

    for (const GCodeProcessorResult::MoveVertex& move : m_result.moves) {
        // Only extrusion counts as "use". A travel or a retract with a tool does not mean the tool
        // still has work to do, and counting them would keep hotends alive for nothing.
        if (move.type != EMoveType::Extrude)
            continue;
        const size_t t = static_cast<size_t>(move.extruder_id);
        if (t < tools && move.gcode_id > m_neotko_toolsleep_last_use[t])
            m_neotko_toolsleep_last_use[t] = move.gcode_id;
    }

    // NEOTKO_TOOLSLEEP_TAG — count the tools that actually extrude. A single-tool print never
    // emits a toolchange, so OozePrevention never writes a ";cooldown" M104, so nothing can be
    // rewritten: reporting that as a failure below was a warning about something that cannot happen.
    for (size_t t = 0; t < tools; ++t)
        if (m_neotko_toolsleep_last_use[t] > 0)
            ++m_neotko_toolsleep_tools_used;

    if (NeoDebug::enabled(NeoDebug::TOOLORDER)) {
        std::string msg = std::string("[TOOLSLEEP] on") + (m_neotko_toolsleep_deep ? " (deep)" : "") +
                          ". tools that extrude: " + std::to_string(m_neotko_toolsleep_tools_used) +
                          ". last extruding line per tool:";
        for (size_t t = 0; t < tools; ++t)
            msg += " T" + std::to_string(t) + "=" + std::to_string(m_neotko_toolsleep_last_use[t]);
        NeoDebug::write(NeoDebug::TOOLORDER, msg);
    }
}

// Rewrites a standby "M104 S<idle> T<n>" into "M104 S0 T<n>" when line_id is already past that
// tool's last extruding move. Returns true if the line was changed.
//
// Deliberately a REWRITE and not an insertion: inserting lines would shift the line/time bookkeeping
// that ExportLines and the M73 estimates depend on. Rewriting keeps every count intact.
//
// Note this needs a standby M104 to exist at all, which on the U1 means ooze_prevention (the
// Snapmaker process profiles ship it on, with standby_temperature_delta -150 => ~70 C). If no
// standby command is ever emitted, nothing is rewritten and the count stays at 0 — reported below.
bool GCodeProcessor::neotko_toolsleep_rewrite_line(std::string& gcode_line, unsigned int line_id)
{
    if (!m_neotko_toolsleep_enabled || m_neotko_toolsleep_last_use.empty())
        return false;
    ...
    float tool_f = 0.f;
    float temp_f = 0.f;
    // An M104 without an explicit T is aimed at the active tool, which by definition is not the one
    // being parked. Leave it alone.
    if (!gline.has_value('T', tool_f) || !gline.has_value('S', temp_f))
        return false;
    if (temp_f <= 0.f)
        return false;  // already off

    // 🚨 The ";cooldown" marker is what makes this line a PARK, and it is not optional.
    // OozePrevention::pre_toolchange is the only thing that emits it (GCode.cpp:263). Every other
    // M104 with a T in the file means the opposite, and rewriting one of those breaks the print:
    //   - "M104 S220 T2 ; set nozzle temperature" right after a T2 pick-up is the working
    //     temperature of the tool about to print. Setting it to 0 makes that tool print while it
    //     cools down.
    //   - "M104 T0 S140" and "M104 T0 S130" in Snapmaker's start gcode are the preheat for
    //     auto-feed and flow calibration, before any of this has a say.
    // Measured on WALL-allON.gcode (s294): without this check, 554 of 858 rewrites were wrong.
    if (gcode_line.find("cooldown") == std::string::npos)
        return false;

    const int tool = static_cast<int>(tool_f);
    if (tool < 0 || static_cast<size_t>(tool) >= m_neotko_toolsleep_last_use.size())
        return false;
    // In deep sleep a tool is powered down at every park, so it must stay eligible all print long.
    if (!m_neotko_toolsleep_deep && m_neotko_toolsleep_done[tool])
        return false;

    const unsigned int last_use = m_neotko_toolsleep_last_use[tool];
    // last_use == 0: the tool never extrudes, so the file's own start gcode already decided not to
    // use it (the U1 start gcode gates every heat-up on is_extruder_used[n]). Do not touch it.
    if (last_use == 0)
        return false;

    // Base rule: only once the tool has no extrusion left. A strict comparison against the parsed
    // line numbering, not a heuristic, because this is the guard that keeps a print alive.
    //
    // "Extra Energy Save" widens it to every park, and hands the timing decision to Orca's own
    // preheat backtrace, which is already the component that knows how long each stretch of gcode
    // takes. When it inserts the preheat for a tool it removes that tool's earlier cooldown if it
    // falls inside the preheat window (the line replacer in process_line_T). So:
    //   short gap  -> the preheat window covers this line, it gets removed, the tool never cools;
    //   long gap   -> the shutdown stands, and the preheat still starts preheat_time before the
    //                 toolchange, which on the U1 is 30 s against a measured 22 s ramp (s277).
    // Either way there is no wait, and the threshold is preheat_time itself rather than a number
    // invented here that could drift away from what the preheat actually does.
    // This is why the ";cooldown" marker below must be preserved: it is what the replacer matches.
    if (!m_neotko_toolsleep_deep && line_id <= last_use)
        return false;

    // Surgical edit: swap the S token's value for 0 and keep everything else on the line, including
    // the ";cooldown" marker that the preheat backtrace looks for.
    ...
    // Drop the trailing newline(s) so the marker lands before them, then put them back.
    std::string eol;
    while (!tail.empty() && (tail.back() == '\n' || tail.back() == '\r')) {
        eol.insert(eol.begin(), tail.back());
        tail.pop_back();
    }
    const char* why = (line_id > last_use) ? " ; neotko: tool off, not used again"
                                          : " ; neotko: tool off while parked";
    gcode_line = gcode_line.substr(0, s_pos) + "S0" + tail + why + eol;

    m_neotko_toolsleep_done[tool] = true;
    ++m_neotko_toolsleep_rewrites;

    if (NeoDebug::enabled(NeoDebug::TOOLORDER))
        NeoDebug::write(NeoDebug::TOOLORDER,
                        std::string("[TOOLSLEEP] T") + std::to_string(tool) + " off at line " +
                            std::to_string(line_id) + " (last extrusion line " + std::to_string(last_use) +
                            (line_id > last_use ? ", final)" : ", parked — the preheat may undo this)"));
    return true;
}
...
void GCodeProcessor::run_post_process()
{
    ...
    // NEOTKO_TOOLSLEEP_TAG s294 — build the per-tool "last extruding line" table before the write
    // pass, so the single forward pass below can answer "does this tool come back?".
    neotko_toolsleep_prepare();
...

    {
        ...
        for (;;) {
            ...
            while (it != it_bufend || (eof && !gcode_line.empty())) {
                ...
                if (eol) {
                    ++line_id;
                    // NEOTKO_TOOLSLEEP_TAG s294 — rewrite a parked tool's standby M104 to S0 when it
                    // has no extrusion left. Must run BEFORE export_lines.update(), which measures
                    // and stores the line; editing it afterwards would desync the byte offsets.
                    neotko_toolsleep_rewrite_line(gcode_line, line_id);
    ...
    // NEOTKO_TOOLSLEEP_TAG s294 — the toggle is on but nothing was switched off. The usual cause is
    // that no standby M104 exists to rewrite (ooze_prevention off), so say it out loud instead of
    // letting the user believe hotends are being powered down when they are not.
    if (m_neotko_toolsleep_enabled) {
        // Only a print that uses MORE THAN ONE tool can have a parked hotend at all. With a single
        // tool there is no toolchange, so OozePrevention never emits the ";cooldown" M104 this
        // rewrites, so the count is always 0 — and warning about it was telling the user something
        // had failed when in fact there was nothing to switch off.
        if (m_neotko_toolsleep_rewrites == 0 && m_neotko_toolsleep_tools_used > 1) {
            const std::string warning = "Turn off unused hotends fully: no tool could be switched off. "
                                        "This needs a standby temperature command to rewrite, which the printer "
                                        "only emits when Ooze prevention is enabled in Print settings.";
            BOOST_LOG_TRIVIAL(warning) << warning;
            if (m_print != nullptr)
                m_print->active_step_add_warning(PrintStateBase::WarningLevel::NON_CRITICAL, warning);
            // The same text has been seen twice in the notification centre on a two-object plate.
            // active_step_add_warning() de-duplicates by message within the active step, so the
            // second copy comes from somewhere else (a second export pass, most likely). Count the
            // emissions here so the next occurrence says how many times this line actually ran,
            // instead of guessing at the fix.
            if (NeoDebug::enabled(NeoDebug::TOOLORDER))
                NeoDebug::write(NeoDebug::TOOLORDER, "[TOOLSLEEP] warning emitted (no tool switched off)");
        }
        if (NeoDebug::enabled(NeoDebug::TOOLORDER))
            NeoDebug::write(NeoDebug::TOOLORDER, "[TOOLSLEEP] done, tools that extrude: " +
                                                     std::to_string(m_neotko_toolsleep_tools_used) +
                                                     ", tools switched off: " +
                                                     std::to_string(m_neotko_toolsleep_rewrites));
    }
