...
{
    // NEOTKO_NEOARACHNE_TAG Inc3-fix (port s134) — walk perimeters in STORED order, do NOT
    // chain-reorder the top level. The real GCode::extrude_perimeters iterates region.perimeters in
    // stored order (it does NOT call chain_and_reorder — only extrude_infill/extrude_support do), so
    // the wall_sequence ordering that Plan::run encoded via std::rotate survives to the gcode. The
    // previous flat chain_and_reorder here re-sorted by proximity and discarded wall_sequence, making
    // the preview show outer/inner while the gcode (correctly) did inner/outer. Per-island buckets are
    // no_sort=true so their internal order is honoured by walk_chain below, matching the emitter.
