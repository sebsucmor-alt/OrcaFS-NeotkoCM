...
ToolOrdering::ToolOrdering(const PrintObject &object, unsigned int first_extruder, bool prime_multi_material)
{
    ...
    this->fill_wipe_tower_partitions(object.print()->config(), object.layers().front()->print_z - object.layers().front()->height, max_layer_height, "single-obj");
...
void ToolOrdering::fill_wipe_tower_partitions(const PrintConfig &config, coordf_t object_bottom_z, coordf_t max_layer_height, const char* call_context)
{
    if (m_layer_tools.empty())
        return;
    ...
    // NEOTKO_MULTIPASS_TAG_START — second sublayer zero-reset, AFTER backward propagation.
    // The backward pass above copies wipe_tower_partitions from subsequent real layers into
    // sublayer entries (because they were zeroed before the pass, so std::max picks the
    // non-zero value from the next real layer). This would make is_mp_sublayer layers
    // satisfy (wipe_tower_partitions > 0), giving them has_wipe_tower=true and causing
    // next_layer() to be called for them in the GCode pipeline — desynchronizing m_layer_idx
    // from the WipeTower plan (Bug: layer=136 out-of-bounds with 2 objects + MultiPass).
    // Re-zeroing here prevents backward-propagation bleed-through into sublayer entries.
    //
