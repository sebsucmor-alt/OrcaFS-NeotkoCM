...
// Neotko Surface ColorStitch + MultiPass Blend — Implementation
...
#include <algorithm>
#include <limits>
#include <set>      // NEOTKO_COLORSTITCH: unique-tool check in build_tool_list_from_pattern
...
#include <numeric>  // NEOTKO_COLORSTITCH s58: std::iota for lane_mode sort indices
...
// ColorStitch cycles through this list to reproduce the correct blend ratio.
//
...
// ---------------------------------------------------------------------------
// NEOTKO_COLORSTITCH_TAG — s58: lane distribution helpers (modes 1/2/3) moved to
// ColorStitch.hpp so GCode.cpp can reuse them for PathBlend. See the header
    ...
    size_t                      num_physical = 0)
{
    ...
    for (char c : pattern) {
        if (c >= '1' && c <= '4') {
            // NEOTKO_COLORSTITCH_TAG — s58: preserve repetitions for physical tools.
            ...
            // at 33%/33%/33%. ColorStitch cycles through the list, so the recipe IS the pattern.
            ...
            if (mf) {
                ...
                if (NeoDebug::enabled(NeoDebug::COLORSTITCH)) {
                    ...
                    _s << "]";
                    NeoDebug::write(NeoDebug::COLORSTITCH, _s.str());
                }
                // Append without dedup — repetitions carry the weighting.
    ...
    // Fallback check: need at least 2 UNIQUE physical tools (repetitions don't count).
    {
        ...
        if (unique_tools.size() < 2) {
            NEOTKO_LOG(COLORSTITCH, "PATTERN_FALLBACK pattern=\"" << pattern
...
// ---------------------------------------------------------------------------
bool ColorStitch::should_process_role(ExtrusionRole role, int surface)
{
    switch (surface) {
...
// ---------------------------------------------------------------------------
int ColorStitch::dominant_painted_slot_in_z_range(const PrintObject* po,
                                                       ...
                                                       bool downward)
{
    ...
    int counts[ModelVolume::COLORSTITCH_SLOT_COUNT] = {0};   // NEOTKO_COLORSTITCH_TAG s112: 16→31
    ...
    for (const ModelVolume* mv : mo->volumes) {
        ...
        for (int slot = 1; slot < ModelVolume::COLORSTITCH_SLOT_COUNT; ++slot) {   // NEOTKO_COLORSTITCH_TAG s112
            ...
            // ColorStitch de esa capa NO se generaba. Fill.cpp (FOOTPRINT_CLIP) ya los
            ...
            {
                const int _pid = mv->colorstitch_slot_to_profile_id[slot];
                if (_pid == 0 ||
                    Slic3r::SurfaceEffectProfileManager::get().find(_pid) == nullptr)
                    continue;
            }
            const indexed_triangle_set its = mv->colorstitch_paint_facets.get_facets(
    ...
    for (int s = 1; s < ModelVolume::COLORSTITCH_SLOT_COUNT; ++s)   // NEOTKO_COLORSTITCH_TAG s112
...
ExPolygons ColorStitch::painted_footprint_in_z_range(const PrintObject* po, int slot,
                                                         double z_min, double z_max,
                                                         bool downward)
{
    ExPolygons out;
    if (!po || slot <= 0 || slot >= ModelVolume::COLORSTITCH_SLOT_COUNT) return out;   // NEOTKO_COLORSTITCH_TAG s112
    const ModelObject* mo = po->model_object();
    if (!mo) return out;
    ...
    if (_ms > 50.0)
        NEOTKO_LOG(COLORSTITCH, "SLOW_UNION painted slot=" << slot
            << " band=[" << z_min << "," << z_max << "]"
...
std::vector<int> ColorStitch::enumerate_painted_slots_in_z_range(
    const PrintObject* po, double z_min, double z_max, bool downward)
{
    ...
    bool present[ModelVolume::COLORSTITCH_SLOT_COUNT] = {false};   // NEOTKO_COLORSTITCH_TAG s112: 16→31
...
// their signatures.
int ColorStitch::painted_slot_owner_volume_index_in_z_range(const PrintObject* po, int slot,
                                                                  double z_min, double z_max,
                                                                  bool downward)
{
    if (!po || slot <= 0 || slot >= ModelVolume::COLORSTITCH_SLOT_COUNT) return -1;
    const ModelObject* mo = po->model_object();
    if (!mo) return -1;
...
//
// A sticker (ModelObject::colorstitch_stickers) is a 1-colour SVG shape placed on
...
bool ColorStitch::object_has_any_colorstitch_stickers(const ModelObject* mo)
{
    if (!mo) return false;
...
std::vector<size_t> ColorStitch::enumerate_stickers_in_z_range(
    const PrintObject* po, double z_min, double z_max)
{
    std::vector<size_t> out;
    if (!po) return out;
    const ModelObject* mo = po->model_object();
    if (!mo || mo->colorstitch_stickers.empty()) return out;
    ...
    for (size_t k = mo->colorstitch_stickers.size(); k-- > 0; ) {
        const ColorStitchSticker& st = mo->colorstitch_stickers[k];
    ...
    if (!out.empty())
        NEOTKO_LOG(PROFILE, "STICKER_ENUM band=[" << z_min << "," << z_max << "]"
            << " pile=" << mo->colorstitch_stickers.size()
            << " in_band=" << out.size() << " (top-down)");
    return out;
}

Polygons ColorStitch::sticker_rings_in_transform(const ColorStitchSticker& sticker,
...
ExPolygons ColorStitch::sticker_footprint_slice_frame(const ColorStitchSticker& sticker,
...
//
// Fix: mirror the colorstitch `painted_footprint_in_z_range` filter — keep ONLY
...
ExPolygons ColorStitch::mmu_painted_footprint_in_z_range(
    const PrintObject* po, double z_min, double z_max, bool downward)
{
    ExPolygons out;
    if (!po) return out;
    ...
    const double _ms = std::chrono::duration<double, std::milli>(
                           std::chrono::steady_clock::now() - _t0).count();
    if (_ms > 50.0)
        NEOTKO_LOG(COLORSTITCH, "SLOW_UNION mmu"
            << " band=[" << z_min << "," << z_max << "]"
...
ColorStitch::CoexistBandMasks ColorStitch::coexist_masks_in_z_range(
    const PrintObject* po, double z_min, double z_max, bool downward)
{
...
// Channel COLORSTITCH (`/tmp/neotko_colorstitch.log`), grep MMU_COEXIST.
void ColorStitch::log_coexist_band(const PrintObject* po, double z_min, double z_max,
                                       bool downward, const char* tag)
{
    ...
    // COLORSTITCH apagado incluido. O sea: desde s234 todo usuario con pintura de sandwich
    ...
    if (!NeoDebug::enabled(NeoDebug::COLORSTITCH)) return;
    if (!po) return;
    const ModelObject* mo = po->model_object();
    if (!mo) return;
    ...
    for (const ModelVolume* mv : mo->volumes) {
        ...
        if (!mv->colorstitch_paint_facets.empty())  any_cm = true;
    }
    ...
    NEOTKO_LOG(COLORSTITCH, "MMU_COEXIST " << (tag ? tag : "?")
        << " z=[" << z_min << "," << z_max << "]"
        << (downward ? " dir=down" : " dir=up")
...
// Consulted identically from ColorStitch::assign_and_group_tools (per-EEC),
...
bool ColorStitch::mmu_governs_xy(
    const PrintObject* po,
    ...
    double z_min, double z_max)
{
    ...
    // below (now unreachable) so a gated MMU-governs mode can be reintroduced.
    {
        ...
        if (_sandwich_always_governs) {
            NEOTKO_LOG(COLORSTITCH, "s91/coexist mmu_governs_xy z=[" << z_min << ","
...
bool ColorStitch::mmu_governs_surface(
...
// of the same object).
int ColorStitch::profile_id_for_slot(const PrintObject* po, int slot)
{
    if (!po || slot <= 0 || slot >= ModelVolume::COLORSTITCH_SLOT_COUNT) return 0;   // NEOTKO_COLORSTITCH_TAG s112
    const ModelObject* mo = po->model_object();
    if (!mo) return 0;
    for (const ModelVolume* mv : mo->volumes) {
        if (!mv || !mv->is_model_part()) continue;
        if (mv->colorstitch_slot_to_profile_id[slot] != 0)
            return mv->colorstitch_slot_to_profile_id[slot];
    }
    return 0;
}
...
int ColorStitch::profile_id_for_slot_in_volume(const PrintObject* po, int slot, int vol_idx)
{
    if (!po || slot <= 0 || slot >= ModelVolume::COLORSTITCH_SLOT_COUNT) return 0;
    const ModelObject* mo = po->model_object();
    if (!mo) return 0;
    if (vol_idx >= 0 && vol_idx < (int)mo->volumes.size()) {
        const ModelVolume* mv = mo->volumes[vol_idx];
        if (mv && mv->is_model_part() && mv->colorstitch_slot_to_profile_id[slot] != 0)
            return mv->colorstitch_slot_to_profile_id[slot];
    }
...
bool ColorStitch::object_has_any_colorstitch_paint(const ModelObject* mo)
{
    if (!mo) return false;
    for (const ModelVolume* mv : mo->volumes) {
        if (!mv || !mv->is_model_part()) continue;
        ...
        // facets but does NOT clear colorstitch_slot_to_profile_id (slots stay mapped),
...
bool ColorStitch::object_painter_wants_penu(const ModelObject* mo)
{
    if (!mo) return false;
    ...
    for (const ModelVolume* mv : mo->volumes) {
        ...
        for (int s = 1; s < ModelVolume::COLORSTITCH_SLOT_COUNT; ++s) {   // NEOTKO_COLORSTITCH_TAG s112
            ...
            // surface sólo se ponía según los pases ColorStitch → se perdían penu Solid/PB
            ...
            // ColorStitch — surface 0=Both / 2=Penu only → wants penu.
            if (p->colorstitch.present) {
                auto it = p->colorstitch.kv.find("interlayer_colormix_surface");
...
bool ColorStitch::object_painter_wants_bottom(const ModelObject* mo)
{
    if (!mo) return false;
    auto& mgr = Slic3r::SurfaceEffectProfileManager::get();
    std::set<int> profile_ids_seen;
...
//
// Behavior mirrors ColorStitch.cpp gv consumption + ToolOrdering.cpp
// `tools_for_role` + `parse_colorstitch_pattern_1based`.
std::vector<unsigned int> ColorStitch::painted_profile_tools_1based(
    const SurfaceEffectProfile& p, bool top_role)
{
    std::vector<unsigned int> out;
    if (!p.colorstitch.present) return out;

    const auto& kv = p.colorstitch.kv;
...
int ColorStitch::assign_and_group_tools(
    ...
    NEOTKO_LOG(COLORSTITCH, "ENTRY layer=" << layer_idx
    ...
    //
    // If the object has ANY painted slot (any volume's colorstitch_slot_to_profile_id
    ...
    //   * a layer/role only gets colorstitch if a painted profile resolves there
    ...
    // partially-MMU object now get sandwich/ColorStitch normally.
    ...
    if (_s91_obj_has_mmu_paint)
        NEOTKO_LOG(COLORSTITCH, "s91/coexist SCM ENTRY layer=" << layer_idx
    ...
    const bool painter_mode_obj = object_has_any_colorstitch_paint(model_object)
        || object_has_any_colorstitch_stickers(model_object);
    ...
    // puntero del mo para cruzarlo con el PAINT_SLOT de PrintObjectSlice.
    {
        ...
        if (model_object)
            for (const ModelVolume* _mv : model_object->volumes) {
                ...
                ++_nvol;
                if (!_mv->colorstitch_paint_facets.empty()) ++_nfacets;
                for (int _s = 1; _s < ModelVolume::COLORSTITCH_SLOT_COUNT; ++_s)
                    if (_mv->colorstitch_slot_to_profile_id[_s] != 0) ++_npaint;
            }
    ...
    if (print_object && layer_height > 0.0) {
        ...
        // comparten capa/esquina). Si el dominante es un slot SIN colorstitch (p.ej. un
        ...
        // colorstitch.present para ver si hay un slot-stitch eclipsado por el dominante.
        if (NeoDebug::enabled(NeoDebug::PROFILE)) {
            std::string _present;
            for (int _s : ColorStitch::enumerate_painted_slots_in_z_range(
    ...
    for (auto* top_entity : fills.entities) {
        ...
        // EEC is owned by MMU segmentation — skip ColorStitch for it. Other EECs
        ...
        if (_s91_obj_has_mmu_paint) {
            ...
            if (!_eec_pts.empty()) {
                ...
                if (mmu_governs_xy(print_object, _eec_xy,
                    ...
                    NEOTKO_LOG(COLORSTITCH, "s91/coexist MMU_GOVERNS site=SCM"
                        << " layer=" << layer_idx
        ...
        // Skip if already colorstitch-encoded (mm3_per_mm >= 10.0)
        {
        ...
        // capa y el dominante cae en un slot sin colorstitch (p.ej. un gradiente solid), el
        ...
        const bool painted_override =
            ...
            || (eff_profile && eff_profile->colorstitch.present);
        ...
        // NEOTKO_COLORSTITCH_TAG — s61: per-role gradient view.
        ...
        // The profile's ColorStitch payload was snapshot-taken with the full key
        ...
        // config ES cm_eff = región + pass.colorstitch.kv (per-pase, fuente de verdad);
        ...
        if (painted_override && !config_has_pass_override) {
            const auto& kv = eff_profile->colorstitch.kv;
        ...
        // NEOTKO_COLORSTITCH_TAG — s60: mode-aware tool resolution.
        ...
        if (painted_override && !config_has_pass_override) {
            ...
            // NEOTKO_COLORSTITCH_TAG — s60: tool_c/_d default to -1 ("off") in the
        ...
        if (n_paths > 1) {
            ...
            NEOTKO_LOG(COLORSTITCH, "MONOTONIC_MODE layer=" << layer_idx
            ...
            if (split_monotonic) {
                ...
                NEOTKO_LOG(COLORSTITCH, "  MONO_SURF_AXIS layer=" << layer_idx
            ...
            if (split_monotonic)
                NEOTKO_LOG(COLORSTITCH, "  MONO_SPLIT layer=" << layer_idx
        ...
        NEOTKO_LOG(COLORSTITCH, "  RAW_LINES layer=" << layer_idx
        ...
        // NEOTKO_COLORSTITCH_TAG — s60: build per-line tool sequence for modes 1-3.
        ...
        // NEOTKO_COLORSTITCH_TAG_START - unique-tool block merge
        ...
        if (NeoDebug::enabled(NeoDebug::COLORSTITCH)) {
            ...
            _c << "]";
            NeoDebug::write(NeoDebug::COLORSTITCH, _c.str());
        }
        ...
        if (NeoDebug::enabled(NeoDebug::COLORSTITCH) && n_paths > 1 && raw_lines.size() >= 2) {
            ...
            NEOTKO_LOG(COLORSTITCH, "MONO_AXIS_DIAG layer=" << layer_idx
                << " role=" << ExtrusionEntity::role_to_string(first_path->role())
                << " n=" << raw_lines.size()
            ...
            NEOTKO_LOG(COLORSTITCH, "  MONO_AXIS_OUTLIERS layer=" << layer_idx
            ...
            for (size_t k = 0; k < cap; ++k) {
                ...
                const LaneVec2 d = lane_direction(raw_lines[i].pl);
                const double   a = ang_norm_pi(std::atan2(d.y, d.x));
                NEOTKO_LOG(COLORSTITCH, "    OUTLIER idx=" << i
        ...
        // NEOTKO_COLORSTITCH_TAG s99 — single-tool short-circuit.
        // If after slot assignment every line ended up in the same tool bucket,
        // the ColorStitch split produced ZERO multi-tool benefit but would still
        ...
        if (unique_tool_order.size() == 1) {
            ...
            NEOTKO_LOG(COLORSTITCH, "SHORT_CIRCUIT layer=" << layer_idx
                << " role=" << ExtrusionEntity::role_to_string(first_path->role())
        ...
        // NEOTKO_COLORSTITCH_TAG s99 — stitch-back REVERTED.
        // The fusion broke the per-line discontinuity that ColorStitch patterns
        ...
        any_modified = true;
        // NEOTKO_COLORSTITCH_TAG_END - unique-tool block merge
    }

    int flags = 0;
    if (any_modified)    flags |= COLORSTITCH_FLAG_MODIFIED;
    if (any_unsplittable) flags |= COLORSTITCH_FLAG_UNSPLITTABLE;
    return flags;
}

// ---------------------------------------------------------------------------
// NEOTKO_COLORSTITCH_TAG — s60 numeric gradient (Step 1 of UX plan).
...
double ColorStitch::colorstitch_easing_apply(double t, int easing, double gamma)
{
...
std::vector<int> ColorStitch::build_custom_bands(
    int n_lines,
    ...
    int tool_d, int cnt_d)
{
    ...
    // NEOTKO_COLORSTITCH_TAG — s60: skip slots with tool index < 0 ("off") in
...
int ColorStitch::estimate_surface_line_count(
    double area_mm2,
...
// ---------------------------------------------------------------------------
void ColorStitch::encode_tool_in_path(ExtrusionPath* path, int tool_idx)
{
...
void ColorStitch::debug_log(
    ...
    if (!NeoDebug::enabled(NeoDebug::COLORSTITCH)) return;
    std::ostringstream _s;
    _s << "=== COLORSTITCH Layer " << layer_idx << " ===\n  Tools: [";
    for (size_t i = 0; i < tools.size(); ++i) _s << (i?",":"") << "T" << tools[i];
    _s << "]";
    for (const auto& pair : grouped) {
        double total_mm = 0.0;
        // NEOTKO_SANDWICH_TAG s119 — per-tool extrusion width trace. Each ColorStitch
...
// ---------------------------------------------------------------------------
// Translate a MixedFilament to a pattern string usable by ColorStitch.
// Priority: manual_pattern > gradient_component_ids > ratio/component build.
// ---------------------------------------------------------------------------
std::string ColorStitch::mixed_filament_to_pattern(const MixedFilament& mf)
{
    if (!mf.manual_pattern.empty())
...
// ---------------------------------------------------------------------------
// Parse mixed_filament_definitions string and build the ColorStitchOption list.
// Enabled + non-deleted mixed entries come first, then physical filaments.
// ---------------------------------------------------------------------------
std::vector<ColorStitchOption> ColorStitch::get_mix_options(
    const std::string&              mixed_defs,
    const std::vector<std::string>& filament_colours)
{
    std::vector<ColorStitchOption> result;
    ...
    if (!mixed_defs.empty()) {
        ...
        for (const auto& mf : mgr.mixed_filaments()) {
            ...
            opt.tool_weights = ColorStitch::extract_recipe_weights(mf, (size_t)num_physical);
...
// ---------------------------------------------------------------------------
std::map<int,float> ColorStitch::extract_recipe_weights(
...
//
// s280 — `multipass_from_profile_payload` (kv → MultiPassConfig sin pasar por un
// PrintRegionConfig temporal) se RETIRA: quedó huérfano al mover Fase F a la ruta
// de `SurfacePassStack::resolve()`, y llevaba meses compilándose sin un solo
// llamante. Su gemelo `painted_profile_tools_1based` (arriba) SÍ sigue vivo.

bool ColorStitch::painted_perim_override_from_profile(
...
PathBlendPassConfig ColorStitch::pathblend_from_profile_payload(
    const SurfaceEffectPayload& payload)
{
...
bool ColorStitch::any_painted_profile_has_perim_override(
    ...
    if (mo && !mo->colorstitch_stickers.empty()) {
        for (const bool is_penu : { false, true }) {
            ...
            for (size_t si : enumerate_stickers_in_z_range(po, z_lo, z_hi)) {
                const ColorStitchSticker& stk = mo->colorstitch_stickers[si];
...
// resolved stack's ColorStitch pass kv, fall back to the raw colorstitch payload.
int ColorStitch::painted_colorstitch_angle_for_slot(const PrintObject* po, int slot, bool penu)
{
    if (po == nullptr || slot <= 0) return -1;
    const int pid = profile_id_for_slot(po, slot);
    return colorstitch_angle_for_profile_id(pid, penu);
}

// NEOTKO_STICKER_TAG — factored out of painted_colorstitch_angle_for_slot so a
// sticker (which has a profile id but no slot) can share the same lookup.
int ColorStitch::colorstitch_angle_for_profile_id(int profile_id, bool penu)
{
    ...
    if (!js.empty()) {
        ...
        for (const SurfacePass& sp : st.passes)
            if (sp.kind == SurfacePassKind::ColorStitch && !sp.colorstitch.kv.empty()) {
                const int a = read(sp.colorstitch.kv);
                if (a != -2) return a;
            }
    }
    if (p->colorstitch.present && !p->colorstitch.kv.empty()) {
        const int a = read(p->colorstitch.kv);
        if (a != -2) return a;
    }
    // NEOTKO_COLORSTITCH_TAG — el perfil EXISTE y es ColorStitch, pero nadie guardó
    // ángulo (el panel sólo escribe la clave cuando el usuario la toca). Eso es el
    // caso por defecto, y su valor es el default, NO auto. Antes se devolvía -1 aquí
    // y una banda pintada recién creada se rebanaba con el ángulo alternante mientras
    // el preview la dibujaba a 45 fijo: ése era el bug de "Prepare no casa con el slice"
    // por el camino del painter/sticker.
    // Los `return -1` de arriba (sin profile_id, o perfil no encontrado) se quedan
    // como estaban: ahí NO sabemos nada, y forzar un ángulo sería inventar.
    // El penu no necesita caso aparte: el lambda `read` ya cae a la clave del top
    // cuando la suya falta, que es lo que significa heredar.
    return COLORSTITCH_DEFAULT_ANGLE_DEG;
}
// NEOTKO_PROFILE_TAG_END


// NEOTKO_NEOWEAVING_PORT_TAG_START — WAVESUPPORT_PLAN.md Fase 1: NeoweaveEngine ported from
// FULLSPECTRUM095 (legacy) ColorStitch.cpp:2313-2527. All Z-motion computation lives here;
...
// so existing iterating callers (GCode COLORSTITCH_HOOK pre-seed, ToolOrdering,
...
std::string SurfacePassStack::to_json() const
{
    ...
    for (const auto& p : passes) {
        ...
        e["colormix"]    = sandwich_payload_to_json(p.colorstitch);
...
SurfacePassStack SurfacePassStack::from_json(const std::string& text)
{
    ...
    for (const auto& e : root["passes"]) {
        ...
        if (e.contains("colormix"))   p.colorstitch  = sandwich_payload_from_json(e["colormix"]);
                                                          ...
                                                          ExtrusionRole role)
{
    ...
    // MultiPass > ColorStitch > PathBlend mirrors the FASE 2 dispatch order.
    ...
    if (mp_on) {
        ...
        if (ColorStitch::should_process_role(role, mp.surface)) {
    ...
    // ColorStitch legacy → single full-height ColorStitch pass. Empty kv → the engine
    ...
    if (cfg.interlayer_colormix_enabled.value) {
        ...
        if (want) {
            ...
            p.ratio = 1.0;
            p.colorstitch.present = true;     // kv empty → preset-config fallback
            st.passes.push_back(std::move(p));
            st.enabled = true;
            return st;
        }
...
ColorStitch::eec_to_tool_buckets(const ExtrusionEntityCollection& encoded,
                                     int default_tool)
{
    std::vector<std::pair<int, ExtrusionEntityCollection>> buckets;
...
std::vector<PBBand> compute_pb_bands(
    ...
    bool                       want_cap,
    int                        target_k,
    double                     min_printable_h)
{
    std::vector<PBBand> out;
    if (H <= 0.0 || min_band_h <= 0.0) return out;

    // NEOTKO_PATHBLEND_TAG s282 — split the old single min_band_h into two roles.
    // `min_band_h` now only quantises the ramp (how many bands); `min_h_phys` is
    // the physical floor every emitted bead must clear. Passing 0 restores the
    // pre-s282 behaviour where one number did both jobs.
    const double min_h_phys = (min_printable_h > 0.0) ? min_printable_h : min_band_h;

    // Clamp config values against physical limits. The PathBlend UX already
    // enforces floor >= 0.01 and (Full) mid_end <= H - 0.04; min_h_phys is the
    // last-resort safety floor on any bead this pass emits.
    const float floor_mm   = std::max(static_cast<float>(min_h_phys), std::max(0.01f, pb.floor_mm));
    const float mid_end_mm = (pb.mode == PathBlendPassConfig::Mode::Full)
        ? std::min(pb.mid_end_mm, static_cast<float>(H - min_h_phys))
    ...
    for (int k = 0; k < K_ramp; ++k) {
        ...
        // NEOTKO_PATHBLEND_TAG s282 — the cap is the thin end. This is the bead
        // that starved in PathBlend-Angle.gcode (H down to 0.0133 mm ⇒ 0.32 mm³/s
        // at 70 mm/s, ~6% of nominal). Floor it on the PHYSICAL limit, not on the
        // quantisation step.
        const double h_cap_mm    = std::max(min_h_phys, H - ramp_at_mid);
        PBBand c;
        c.is_cap = true;
        c.t_lo   = r.t_lo;
        c.t_hi   = r.t_hi;
