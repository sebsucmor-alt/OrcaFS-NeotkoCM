...
void PrintConfigDef::init_fff_params()
{
    ...
    */
    ...
    def->tooltip  = L("Adds this many extra perimeter walls to painted (multi-material) regions only, "
                    "on top of the object's normal wall count. Useful to keep a small painted mark "
                    "fully solid without thickening the whole object's walls. Zero disables this feature.");
    def->min      = 0;
    def->max      = 8;
    def->category = L("Advanced");
    def->mode     = comAdvanced;
...
//    def->min = 0;
    ...
    def->category = L("Strength");
    def->tooltip = L("Density of the solid infill layer immediately preceding the top layers. "
                     "Allows for a smoother transition between sparse infill and solid top surfaces.");
    def->sidetext = L("%");
    def->min = 0;
    def->max = 100;
    def->set_default_value(new ConfigOptionPercent(100));

    def = this->add("penultimate_top_layers", coInt);
    def->label = L("Penultimate top layers");
    def->category = L("Strength");
    def->tooltip = L("Number of solid infill layers immediately below the top surface layers. "
                     "These use reduced density for smoother transition and faster printing.");
    def->min = 0;
    def->max = 20;
    def->set_default_value(new ConfigOptionInt(0));

    def = this->add("penultimate_infill_speed", coFloat);
    def->label = L("Penultimate infill speed");
    def->category = L("Speed");
    def->tooltip = L("Speed for penultimate solid infill layers (layers below top surface). "
                     "If set to 0, uses top_surface_speed instead.");
    def->sidetext = L("mm/s");
    def->min = 0;
    ...
    def->category = L("Quality");
    def->tooltip = L("When enabled together with MultiPass Blend, creates a diagonal color gradient "
                     "within each top surface path by varying Z height and extrusion flow simultaneously. "
                     "Pass 0 ramps Z from layer bottom to top (flow 0→1); Pass 1 stays at nominal Z "
                     "with complementary flow (1→0). Requires MultiPass Blend enabled.");
    def->mode = comDevelop;
    def->set_default_value(new ConfigOptionBool(false));

    def = this->add("path_gradient_segments", coInt);
    def->label = L("Path Gradient Segments");
    def->category = L("Quality");
    def->tooltip = L("Reserved for future path subdivision. Currently unused — gradient uses "
                     "existing polyline points.");
    def->min  = 4;
    def->max  = 256;
    def->mode = comDevelop;
    def->set_default_value(new ConfigOptionInt(32));

    def = this->add("path_gradient_min_flow_pct", coInt);
    def->label = L("Path Gradient Min Flow %");
    def->category = L("Quality");
    def->tooltip = L("Minimum flow percentage at the near-zero end of a PathBlend pass. "
                     "Prevents gaps from truly zero extrusion. Range 0-30. Default 5.");
    def->min  = 0;
    def->max  = 30;
    def->sidetext = L("%");
    def->mode = comDevelop;
    ...
    def->tooltip = L("Oscillates the nozzle in Z while printing top surface infill lines, creating an "
                     "interlaced pattern that mechanically bonds adjacent passes.\n\n"
    ...
    def->category = L("Quality");
    def->tooltip = L("Wave: continuous triangular-wave Z oscillation along the entire path.\n\n"
                     "Linear: each individual line ramps linearly from -amplitude to +amplitude "
    ...
    def = this->add("neoweave_filter", coEnum);
    def->label = L("Solid infill filter");
    def->category = L("Quality");
    def->tooltip = L("Controls which solid infill types are affected by surface neoweaving.\n\n"
                     "Top only: Only top surface layers (stTop + penultimate) — recommended.\n\n"
                     "All: All solid infill types — legacy behavior.");
    def->enum_keys_map = &ConfigOptionEnum<NeoweaveFilter>::get_enum_values();
    def->enum_values.push_back("all");       // index 0 = NeoweaveFilter::All
    def->enum_values.push_back("top_only");  // index 1 = NeoweaveFilter::TopOnly
    def->enum_labels.push_back(L("All solid infill"));
    ...
    def->category = L("Quality");
    def->tooltip = L("First extruder in the mixing cycle (T0-T3, 0-based).");
    def->sidetext = L("T0-T3");
    def->min = 0;
    def->max = 3;
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionInt(0));

    def = this->add("interlayer_colormix_tool_b", coInt);
    def->label = L("Second tool (B)");
    def->category = L("Quality");
    def->tooltip = L("Second extruder in the mixing cycle (T0-T3).");
    def->sidetext = L("T0-T3");
    def->min = 0;
    def->max = 3;
    def->mode = comAdvanced;
    ...
    def = this->add("interlayer_colormix_tool_c", coInt);
    def->label = L("Third tool (C, -1 = off)");
    def->category = L("Quality");
    def->tooltip = L("Third extruder. Set to -1 to use only two tools.");
    def->sidetext = L("T0-T3 or -1");
    def->min = -1;
    def->max = 3;
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionInt(-1));

    def = this->add("interlayer_colormix_tool_d", coInt);
    def->label = L("Fourth tool (D, -1 = off)");
    def->category = L("Quality");
    def->tooltip = L("Fourth extruder. Set to -1 to use only two or three tools.");
    def->sidetext = L("T0-T3 or -1");
    def->min = -1;
    def->max = 3;
    def->mode = comAdvanced;
    ...
    def->label = L("Top surface pattern");
    def->category = L("Quality");
    def->tooltip = L("Tool sequence for top surface lines. Digits 1-4 (1=T0, 2=T1, 3=T2, 4=T3).\n"
                     "Examples: \"12\" = alternate T0/T1, \"1221\" = T0,T1,T1,T0 then repeat.");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionString("12"));

    def = this->add("interlayer_colormix_pattern_penultimate", coString);
    def->label = L("Penultimate layer pattern");
    def->category = L("Quality");
    ...
    def->label = L("Enable MultiPass Blend");
    def->category = L("Quality");
    def->tooltip = L("Re-prints the top/penultimate surface N times with different tools "
                     "and reduced line width, creating a physical color blend.");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));

    def = this->add("multipass_surface", coInt);
    def->label = L("Apply MultiPass to");
    def->tooltip = L("0 = Both (top + penultimate)\n1 = Top only\n2 = Penultimate only");
    def->min = 0; def->max = 2; def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionInt(0));

    def = this->add("multipass_num_passes", coInt);
    def->label = L("Number of passes");
    def->tooltip = L("1, 2 or 3 passes. 1 pass at reduced ratio prints a single light surface layer (glaze / low-flow effect).");
    def->min = 1; def->max = 3; def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionInt(2));

    def = this->add("multipass_tool_1", coInt);
    def->label = L("Pass 1 tool");
    def->sidetext = L("T0-T3"); def->min = 0; def->max = 3; def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionInt(0));

    def = this->add("multipass_tool_2", coInt);
    def->label = L("Pass 2 tool");
    def->sidetext = L("T0-T3"); def->min = 0; def->max = 3; def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionInt(1));

    def = this->add("multipass_tool_3", coInt);
    def->label = L("Pass 3 tool (-1 = off)");
    ...
    def = this->add("multipass_vary_pattern", coBool);
    def->label = L("Alternate pass direction");
    def->tooltip = L("Each successive pass reverses the polyline traversal order (180 deg flip).");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));

    def = this->add("multipass_angle_1", coInt);
    def->label = L("Pass 1 fill angle");
    def->tooltip = L("Custom fill angle for pass 1 (0-359). -1 = follow top surface fill angle.");
    def->sidetext = L("deg  (-1=auto)"); def->mode = comAdvanced;
    def->min = -1; def->max = 359;
    def->set_default_value(new ConfigOptionInt(-1));

    def = this->add("multipass_angle_2", coInt);
    def->label = L("Pass 2 fill angle");
    def->sidetext = L("deg  (-1=auto)"); def->mode = comAdvanced;
    def->min = -1; def->max = 359;
    def->set_default_value(new ConfigOptionInt(-1));

    def = this->add("multipass_angle_3", coInt);
    def->label = L("Pass 3 fill angle");
    def->sidetext = L("deg  (-1=auto)"); def->mode = comAdvanced;
    def->min = -1; def->max = 359;
    def->set_default_value(new ConfigOptionInt(-1));

    def = this->add("multipass_pa_mode", coInt);
    def->label = L("PA mode during MultiPass");
    def->tooltip = L("0 = No changes. 1 = Set PA to custom value. 2 = Set PA to 0.");
    def->mode = comAdvanced;
    def->min = 0; def->max = 2;
    def->set_default_value(new ConfigOptionInt(0));

    def = this->add("multipass_pa_value", coFloat);
    def->label = L("PA value during MultiPass");
    def->tooltip = L("Pressure Advance value when PA mode = 1 (custom).");
    def->sidetext = L("mm"); def->mode = comAdvanced;
    def->min = 0.0; def->max = 2.0;
    def->set_default_value(new ConfigOptionFloat(0.1));

    def = this->add("multipass_fan_1", coInt);
    def->label = L("Pass 1 fan speed");
    def->tooltip = L("Fan speed (0-255) for pass 1. -1 = no change.");
    def->sidetext = L("PWM"); def->mode = comAdvanced;
    def->min = -1; def->max = 255;
    def->set_default_value(new ConfigOptionInt(-1));

    def = this->add("multipass_fan_2", coInt);
    def->label = L("Pass 2 fan speed");
    def->sidetext = L("PWM"); def->mode = comAdvanced;
    def->min = -1; def->max = 255;
    def->set_default_value(new ConfigOptionInt(-1));

    def = this->add("multipass_fan_3", coInt);
    def->label = L("Pass 3 fan speed");
    def->sidetext = L("PWM"); def->mode = comAdvanced;
    def->min = -1; def->max = 255;
    def->set_default_value(new ConfigOptionInt(-1));

    def = this->add("multipass_speed_pct_1", coInt);
    def->label = L("Pass 1 speed %");
    def->tooltip = L("Print speed multiplier via M220 for pass 1. 100 = no change.");
    def->sidetext = L("%"); def->mode = comAdvanced;
    def->min = 1; def->max = 200;
    def->set_default_value(new ConfigOptionInt(100));

    def = this->add("multipass_speed_pct_2", coInt);
    def->label = L("Pass 2 speed %");
    def->sidetext = L("%"); def->mode = comAdvanced;
    def->min = 1; def->max = 200;
    def->set_default_value(new ConfigOptionInt(100));

    def = this->add("multipass_speed_pct_3", coInt);
    def->label = L("Pass 3 speed %");
    def->sidetext = L("%"); def->mode = comAdvanced;
    def->min = 1; def->max = 200;
    def->set_default_value(new ConfigOptionInt(100));

    def = this->add("multipass_gcode_start_1", coString);
    def->label = L("Pass 1 start GCode");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionString(""));

    def = this->add("multipass_gcode_start_2", coString);
    def->label = L("Pass 2 start GCode");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionString(""));

    def = this->add("multipass_gcode_start_3", coString);
    def->label = L("Pass 3 start GCode");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionString(""));

    def = this->add("multipass_gcode_end_1", coString);
    def->label = L("Pass 1 end GCode");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionString(""));

    def = this->add("multipass_gcode_end_2", coString);
    def->label = L("Pass 2 end GCode");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionString(""));

    def = this->add("multipass_gcode_end_3", coString);
    def->label = L("Pass 3 end GCode");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionString(""));
