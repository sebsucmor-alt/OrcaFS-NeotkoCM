    ...
    const ConfigOptionResolver &old_config, const ConfigOptionResolver &new_config, const std::vector<t_config_option_key> &opt_keys)
{
    ...
    for (const t_config_option_key &opt_key : opt_keys) {
        if (   opt_key == "brim_width"
            ...
            || opt_key == "neostitch"
            || opt_key == "neostitch_depth"
            || opt_key == "neostitch_flat_length"
            || opt_key == "neostitch_ramp_length"
            || opt_key == "neostitch_period"
            || opt_key == "neostitch_flow"
            || opt_key == "neostitch_skip_layers"
            || opt_key == "neostitch_fill_margin"
            || opt_key == "detect_overhang_wall"
...
//                    shell = union_(shell, true);
                    ...
                    ExPolygons new_internal_solid_bottom;
                    {
                        ...
                        if (!new_penultimate_solid.empty())
                            NEOTKO_LOG(PENULTIMATE, "discover_vertical_shells layer=" << idx_layer
                                << " penultimate_polys=" << new_penultimate_solid.size()
                                << " solid_polys=" << new_internal_solid_bottom.size());
                    }
                    // NEOTKO_MULTIPASS_TAG_END
                        ...
                        if (new_internal_solid.empty()) {
                            ...
                            for (const Surface &surface : neighbor_layerm->fill_surfaces.surfaces)
                                if (surface.surface_type == stPenultimateInternalSolid)
                                    polygons_append(penultimate_polys, to_polygons(surface.expolygon));
                            if (!penultimate_polys.empty() &&
                                !intersection(solid, penultimate_polys, ApplySafetyOffset::Yes).empty())
                                has_penultimate_intersection = true;
                        }
                        // NEOTKO_MULTIPASS_TAG_END
                    }
                    if (new_internal_solid.empty()) {
