...
//    const float INFILL_OVERLAP_OVER_SPACING = 0.3f;
    ...
    // Save the original angle (includes M_PI/2 from _infill_direction) for nesting offset
    const float angle_from_infill_direction = rotate_vector.first;
    ...
    // On odd layers, shift the infill reference point by half a line spacing perpendicular
    // to the fill direction. Lines of layer N then fall into the gaps of layer N-1,
    // creating XY mechanical interlock. Combined with Neoweaving Linear (Feature 6),
    // this gives XY + Z nesting simultaneously.
    // Only applies to FillMonotonic when neotko_interlayer_nesting_enabled is set.
    {
        const bool _nesting_on = this->print_region_config != nullptr
            && this->print_region_config->neotko_interlayer_nesting_enabled.value;
        if (_nesting_on
            && dynamic_cast<const FillMonotonic*>(this) != nullptr
            && this->layer_id != size_t(-1) && (this->layer_id & 1u)) {
            // angle_from_infill_direction has M_PI/2 added; subtract it to recover fill angle θ
            const float angle_before_rotation = angle_from_infill_direction - float(M_PI / 2.f);
            const coord_t half_sp = coord_t(scale_(this->spacing) * 0.5);
            rotate_vector.second += Point(
                coord_t(-std::sin(double(angle_before_rotation)) * double(half_sp)),
                coord_t( std::cos(double(angle_before_rotation)) * double(half_sp)));
        }
    }
