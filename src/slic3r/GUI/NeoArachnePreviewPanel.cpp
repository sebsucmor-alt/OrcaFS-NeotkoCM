...
DynamicPrintConfig live_merged_config(Tab* tab)
{
    ...
    if (pb != nullptr)
        out = pb->full_config();
    if (tab != nullptr) {
        // NEOTKO_NEOARACHNE_TAG Inc3 (port s134) — read this tab's config. For the global Print
        // Settings tab this is the global config; for the per-object tab (TabPrintModel) it is the
        // SELECTED object's merged config, so the preview reflects that object's wall settings. No
        // per-object overlay needed here — the Objects-mode panel already carries the object config.
