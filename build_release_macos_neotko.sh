...
while getopts ":dpa:snt:xbc:j:1DCSHh" opt; do
  case "${opt}" in
    ...
    C )
        ...
        ;;
    D )
        export ORCA_DEBUG_PENULTIMATE="1"
        ;;
    S )
    ...
    h ) echo "Usage: ./build_release_macos.sh [-d]"
        ...
        echo "   -C: Clean slicer build objects before building (forces full recompile, preserves deps)"
        echo "   -CS: Smart clean - removes only object files for sources changed since last build"
        echo "   -D: Enable penultimate layers debug logging (ORCA_DEBUG_PENULTIMATE)"
...
# NEOTKO: -H flag — limit CPU usage to ~50% so the machine stays usable while compiling.
# Uses half the logical cores and runs the build at lower OS priority (nice +10).
# NEOTKO: -j N flag — use exactly N cores at lower priority (e.g. -j 2 for 2 cores).
# -H and -j are mutually exclusive; -j takes precedence if both are set.
if [ -n "$EXPLICIT_JOBS" ] || [ "1" = "$HALF_CPU" ]; then
    TOTAL_CORES=$(sysctl -n hw.logicalcpu)
    if [ -n "$EXPLICIT_JOBS" ]; then
        CHOSEN_CORES="$EXPLICIT_JOBS"
        MODE_LABEL="-j ${CHOSEN_CORES} cores"
    else
        CHOSEN_CORES=$(( TOTAL_CORES / 2 ))
        [ "$CHOSEN_CORES" -lt 1 ] && CHOSEN_CORES=1
        MODE_LABEL="half (${CHOSEN_CORES}/${TOTAL_CORES} cores)"
    fi
    export CMAKE_BUILD_PARALLEL_LEVEL=$CHOSEN_CORES
    if [ -z "$NICE_REEXEC" ]; then
        export NICE_REEXEC=1
        echo "🐢 CPU limit: $MODE_LABEL at lower priority (nice +10)"
        exec nice -n 10 "$0" "$@"
    fi
fi
...
[ "1" = "$HALF_CPU" ] && echo " - CPU: half (${CMAKE_BUILD_PARALLEL_LEVEL} cores, nice +10)"
[ -n "$EXPLICIT_JOBS" ] && echo " - CPU: ${EXPLICIT_JOBS} cores (nice +10)"
echo
...
# NEOTKO: smart clean — removes only .o files for sources changed since last build.
# Triggered by -CS. Falls back to full src/ clean if no build timestamp exists.
function smart_clean_slicer() {
    local _ARCH=$1
    local build_dir="$PROJECT_DIR/build/$_ARCH"
    local src_dir="$PROJECT_DIR/src"
    local marker="$build_dir/.last_build_timestamp"

    if [ ! -f "$marker" ]; then
        echo "⚠️  No build timestamp found for $_ARCH — falling back to full clean..."
        if [ -d "$build_dir/src" ]; then
            rm -rf "$build_dir/src"
            echo "   Removed: $build_dir/src"
        else
            echo "   Nothing to clean in $build_dir/src"
        fi
        return
    fi

    echo "🔍 Smart clean for $_ARCH: scanning sources changed since last build..."

    local changed
    changed=$(find "$src_dir" -newer "$marker" \( -name "*.cpp" -o -name "*.mm" -o -name "*.c" \) 2>/dev/null)

    if [ -z "$changed" ]; then
        echo "   ✅ No changed source files detected — nothing to clean."
        return
    fi
    ...
    while IFS= read -r src_file; do
        ...
        while IFS= read -r obj_file; do
            ...
            rm -f "$obj_file"
            (( count++ )) || true
        done < <(find "$build_dir/src" -name "${base}.o" 2>/dev/null)
    done <<< "$changed"

    if [ "$count" -eq 0 ]; then
        echo "   ✅ No matching .o files found (may not have been compiled yet)."
    else
        echo "   ✅ Removed $count object file(s)"
    fi
}
...
function build_slicer() {
    ...
    for _ARCH in x86_64 arm64; do
        ...
        if [ "$ARCH" == "universal" ] || [ "$ARCH" == "$_ARCH" ]; then
            ...
            # NEOTKO: -D flag — enable penultimate layers debug logging.
            # Forces CMake reconfiguration so the flag is always applied correctly.
            if [ "1" = "$ORCA_DEBUG_PENULTIMATE" ]; then
                echo "⚠️  ORCA_DEBUG_PENULTIMATE enabled - debug logging will be compiled in"
                echo "    Logs will be written to: /tmp/penultimate_debug.log"
                echo "    Forcing CMake reconfiguration to apply debug flags..."
                unset BUILD_ONLY
            fi

            # NEOTKO: -C flag — clean compiled slicer objects before build.
            # -CS (smart clean): removes only .o files for sources changed since last build.
            # -C  (full clean):  removes all src/ object files. Use when -CS is not enough.
            if [ "1" = "$SMART_CLEAN" ]; then
                smart_clean_slicer "$_ARCH"
            elif [ "1" = "$CLEAN_BUILD" ]; then
                echo "🧹 -C flag set: full clean of slicer build objects for $_ARCH (preserving deps)..."
                if [ -d "$PROJECT_BUILD_DIR/src" ]; then
                    rm -rf "$PROJECT_BUILD_DIR/src"
                    echo "   Removed: $PROJECT_BUILD_DIR/src"
                else
                    echo "   Nothing to clean in $PROJECT_BUILD_DIR/src"
                fi
            fi
            ...
            # Update build timestamp for smart clean tracking
            touch "$PROJECT_BUILD_DIR/.last_build_timestamp"
            echo "📌 Build timestamp updated for $_ARCH"
