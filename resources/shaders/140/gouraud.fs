...
in vec3 eye_normal;
in vec3 eye_pos;   // NEOTKO_SMOOTHNORMALS_TAG s229
...
vec3 weave_color(vec3 base)
{
    if (!u_weave_on || u_weave_n <= 0)
        return base;
    // s318 F3 — antes: -x·sin(ángulo) + y·cos(ángulo) sobre la malla cruda. Ahora el eje llega
    // resuelto en CPU al marco de rebanado del motor (u_weave_angle ya no se usa aquí).
    float proj = dot(u_weave_axis, weave_model_pos.xyz);
    ...
    if (!u_weave_dual)
        return b;
    int  idx2 = weave_index2(dot(u_weave2_axis, weave_model_pos.xyz));
    vec3 c2   = vec3(0.0);
    for (int i = 0; i < 64; ++i)
...
void main()
{
    ...
    if (shading_debug_view != 0) {
        ...
        if (shading_debug_view == 1)
            ...
            out_color = vec4(dev, 1.0 - dev, 0.0, 1.0);
        }
    }
}
