...
// s318 — las primitivas del panel (unidad, iconos, filas, avisos…) subieron a GizmoNeotkoStyle.hpp,
// porque el ColorStitch Painter es su segundo usuario. Aquí sólo queda lo que es DE ESTE panel: la
// columna medida sobre sus etiquetas, que se registra al empezar el cuerpo (render_panel_body).
float neo_label_col()
{
    return neo_label_col_for({ _u8L("overhangs"), _u8L("map detail"), _u8L("ghost"), _u8L("grid marks"),
                               _u8L("across"), _u8L("side"), _u8L("edge · patch"), _u8L("edge · foot"),
                               _u8L("lean"), _u8L("footprint") });
}
...
{
    neo_set_label_col_fn(&neo_label_col);   // s318 — ver GizmoNeotkoStyle.hpp
    if (m_zones_dirty)
        rebuild_zone_rows();
