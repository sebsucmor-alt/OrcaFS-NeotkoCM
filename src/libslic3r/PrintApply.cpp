    ...
    int plate_index)
{
    ...
    for (const t_config_option_key &opt_key : current_config.keys()) {
        // NEOTKO #501 s143 — revert the wipe_tower_filament diff-skip. It existed only
        // because ToolOrdering auto-forced the value (now reverted), which made it
        // change spuriously on every plate switch. With the force gone the option is
        // back under user control and participates in the diff normally.
#if 0
...
static t_config_option_keys full_print_config_diffs(const DynamicPrintConfig &current_full_config, const DynamicPrintConfig &new_full_config, int plate_index)
{
    ...
    for (const t_config_option_key &opt_key : new_full_config.keys()) {
        // NEOTKO #501 s143 — revert the wipe_tower_filament diff-skip (see print_config_diffs).
#if 0
        if (opt_key == "wipe_tower_filament")
            continue;
#endif
