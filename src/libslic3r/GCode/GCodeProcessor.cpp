...
// NEOTKO_NEOTOWER_TAG_START — hardening P1
// G91/G90-aware Z extraction. Only returns Z when in absolute positioning mode.
// In relative mode, Z values are skipped (no absolute base to project the delta).
// Without this, scripts like "G91; G1 Z1.5; G90" returned z=1.5 as absolute
// → corrupted m_writer.position.z → crash on next deretraction (Bug 7 / Bug 11).
bool GCodeProcessor::get_last_z_from_gcode(const std::string& gcode_str, double& z)
{
    int  str_size      = (int)gcode_str.size();
    int  start_index   = 0;
    int  end_index     = 0;
    bool is_z_changed  = false;
    bool relative_mode = false;  // false = G90 (absolute), true = G91 (relative)
    ...
    while (end_index < str_size) {
        ...
        if (end_index > start_index) {
            ...
            // Track positioning mode (G90 absolute / G91 relative).
            // M82/M83 affect E axis only — ignored here.
            if (line_str.size() >= 3 && line_str[0] == 'G') {
                if (line_str.compare(0, 3, "G90") == 0 &&
                    (line_str.size() == 3 || line_str[3] == ' ' || line_str[3] == ';')) {
                    relative_mode = false;
                } else if (line_str.compare(0, 3, "G91") == 0 &&
                           (line_str.size() == 3 || line_str[3] == ' ' || line_str[3] == ';')) {
                    relative_mode = true;
                }
            }

            // Parse motion commands for Z values.
                                       ...
                                       || line_str.find("G3 ") == 0))
            {
                ...
                if (z_pos != line_str.npos && z_pos + 2 < line_str.size()) {
                    ...
                    if (pend != c && is_end_of_word(*pend)) {
                        ...
                        // Relative mode: skip — no absolute base to project delta.
                        // Caller's m_writer.position().z() already holds correct pre-script Z.
                    }
                }
            }
        }
